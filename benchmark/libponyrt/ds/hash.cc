...
class HashMapBench: public ::benchmark::Fixture
{
  protected:
    ...
    void clear_elements(size_t del_count, size_t count);
...
void HashMapBench::clear_elements(size_t del_pct, size_t count)
{
  size_t del_count = (del_pct * count) / 100;
  size_t final_count = (del_count > count) ? 0 : count - del_count;

  // clear random items until map size is as small as required
  while(testmap_size(&_map) > final_count)
  {
    size_t index = rand() % _map.contents.size;

    hash_elem_t* d = testmap_clearindex(&_map, index);
    if(d != NULL)
      free_elem(d);
  }
}
...
BENCHMARK_DEFINE_F(HashMapBench, HashSearchOptimized)(benchmark::State& st) {
  // range(2) == % of items to delete at random
  clear_elements(st.range(2), st.range(1));
  hash_elem_t* e1 = get_element();
  size_t map_count = testmap_size(&_map);
  // we're testing our pathological case
  if(map_count == 0)
...
BENCHMARK_REGISTER_F(HashMapBench, HashSearchOptimized)
  ->Args({1, 1<<10, 64, 0})
  ->Args({1, 2<<10, 64, 0})
  ->Args({1, 4<<10, 64, 0})
  ->Args({1, 8<<10, 64, 0})
