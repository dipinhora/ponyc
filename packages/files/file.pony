...
use @read[SSize](fd: I32, buffer: Pointer[None], bytes_to_read: USize) if not windows
use @_write[I32](fd: I32, buffer: Pointer[None], bytes_to_send: I32) if windows
use @writev[SSize](fd: I32, buffer: Pointer[None], num_to_send: I32) if not windows
use @_lseeki64[Off](fd: I32, offset: Off, base: I32) if windows
use @lseek[Off](fd: I32, offset: Off, base: I32) if not windows
...
use @_chsize_s[I32](fd: I32, len: Off) if windows
use @ftruncate[I32](fd: I32, len: Off) if not windows
...
class File
  ...
  fun _get_error(): FileErrNo =>
    ...
    match os_errno
    | EBADF() => return FileBadFileNumber
    | EEXIST() => return FileExists
    | EACCES() => return FilePermissionDenied
    else
      return FileError
    end
  ...
  fun ref line(): String iso^ ? =>
    ...
    while not done do
      ...
      let r = (ifdef windows then
        @_read(_fd, result.cpointer().usize() + offset, bytes_to_read.i32())
      else
        @read(_fd, result.cpointer().usize() + offset, bytes_to_read)
      end).isize()
  ...
  fun ref read(len: USize): Array[U8] iso^ =>
    ...
    if _fd != -1 then
      ...
      let r = (ifdef windows then
        ...
        @read(_fd, result.cpointer(), Size(len))
      end).isize()
  ...
  fun ref read_string(len: USize): String iso^ =>
    ...
    if _fd != -1 then
      ...
      let r = (ifdef windows then
        ...
        @read(_fd, result.cpointer(), Size(result.space()))
      end).isize()
  ...
  fun _write_to_disk(): (Bool, USize, USize) ? =>
    ...
    while writeable and (pending_total > 0)
      ...
      var len = ifdef windows then
        ...
        @writev(_fd, _pending_writev.cpointer().usize() + num_sent,
          num_to_send).isize()
      end
  ...
  fun ref set_length(len: USize): Bool =>
    ...
    if path.caps(FileTruncate) and writeable and (_fd != -1) then
      ...
      let result = ifdef windows then
        @_chsize_s(_fd, Off(len.i64()))
      else
        @ftruncate(_fd, Off(len.i64()))
      end
  ...
  fun ref _seek(offset: Off, base: I32) =>
    """
    Move the cursor position.
    """
    if _fd != -1 then
