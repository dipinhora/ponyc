...
size_t ponyint_pagemap_mem_size()
{
  return atomic_load_explicit(&mem_used, memory_order_relaxed)
    + sizeof(level) + sizeof(root) + sizeof(mem_used) + sizeof(mem_allocated);
}
...
void ponyint_pagemap_set(const void* addr, chunk_t* chunk)
{
  ...
  for(size_t i = 0; i < PAGEMAP_LEVELS; i++)
  {
    ...
    if(node == NULL)
    {
      ...
      atomic_fetch_add_explicit(&mem_allocated, ponyint_pool_size(level[i].size_index), memory_order_relaxed);
#endif
        ...
        memory_order_release, memory_order_acquire))
      {
        ...
        atomic_fetch_sub_explicit(&mem_allocated, ponyint_pool_size(level[i].size_index), memory_order_relaxed);
#endif
      } else {
        node = new_node;
      }
