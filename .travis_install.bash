...
download_llvm(){
  ...
  tar -xvf clang+llvm*
  pushd clang+llvm* && mkdir /tmp/llvm && cp -r ./* /tmp/llvm/
  ln -s "/tmp/llvm/bin/llvm-config" "/usr/local/bin/${LLVM_CONFIG}"
  popd
}
...
download_libunwind(){
  ...
  pushd libunwind-1.2.1 && ./configure --prefix=/usr && make && make install
  popd
  pushd libunwind-1.2.1 && ./configure --prefix=/usr/local && make && make install
  popd
  if [[ "${CROSS_ARCH}" != "" ]]
  then
    echo "Cross building libunwind..."
    pushd libunwind-1.2.1 && ./configure --prefix=/usr/cross --host="${CROSS_TRIPLE}" CC="${CROSS_CC}" CXX="${CROSS_CXX}" CFLAGS="${CROSS_CFLAGS}" LDFLAGS="${CROSS_LDFLAGS}" && make && make install
    popd
  fi
}
...
download_pcre(){
  ...
  pushd pcre2-10.21 && ./configure --prefix=/usr && make && make install
  popd
  if [[ "${CROSS_ARCH}" != "" ]]
  then
    echo "Cross building PCRE2..."
    pushd pcre2-10.21 && ./configure --prefix=/usr/cross --host="${CROSS_TRIPLE}" CC="${CROSS_CC}" CXX="${CROSS_CXX}" CFLAGS="${CROSS_CFLAGS}" LDFLAGS="${CROSS_LDFLAGS}" && make && make install
    popd
    ...
    pushd libressl-2.4.5 && ./configure --prefix=/usr/cross --disable-asm --host="${CROSS_TRIPLE}" CC="${CROSS_CC}" CXX="${CROSS_CXX}" CFLAGS="${CROSS_CFLAGS}" LDFLAGS="${CROSS_LDFLAGS}" && make && make install
    popd
  fi
}
...
set_linux_compiler(){
  ...
  update-alternatives --install /usr/bin/gcc gcc "/usr/bin/$ICC1" 60 --slave /usr/bin/g++ g++ "/usr/bin/$ICXX1"
}
...
case "${CROSS_ARCH}" in
  ...
  "i686")
    ...
    apt-get -qq update
    apt-get install libc6:i386 libc6-dev:i386 linux-libc-dev:i386 zlib1g:i386 zlib1g-dev:i386
    apt-get install g++-6 g++-6-multilib gcc-6-multilib
  ;;

  "armv7-a")
    pushd /tmp
    wget "https://releases.linaro.org/components/toolchain/binaries/6.4-2017.11/arm-linux-gnueabihf/gcc-linaro-6.4.1-2017.11-x86_64_arm-linux-gnueabihf.tar.xz"
    tar xJvf gcc-linaro-6.4.1-2017.11-x86_64_arm-linux-gnueabihf.tar.xz -C /usr/local --strip 1
    arm-linux-gnueabihf-gcc --version
    popd
    wget https://github.com/multiarch/qemu-user-static/releases/download/v2.9.1-1/qemu-arm-static -O /usr/bin/qemu-arm-static
