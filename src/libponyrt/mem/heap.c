...
{
  return mem_used
    + sizeof(sizeclass_empty) + sizeof(sizeclass_init) + sizeof(sizeclass_table) + sizeof(heap_initialgc) + sizeof(heap_nextgc_factor) + sizeof(mem_used) + sizeof(mem_allocated);
}
...
#ifdef USE_MEMTRACK
  uint32_t empty, size_t size, size_t* allocated, size_t* overhead)
#else
...
#endif
{
  ...
  while(chunk != NULL)
  {
    ...
    {
#ifdef USE_MEMTRACK
      *allocated += POOL_ALLOC_SIZE(chunk_t);
      *allocated += POOL_ALLOC_SIZE(block_t);
...
#ifdef USE_MEMTRACK
static chunk_t* sweep_large(chunk_t* chunk, size_t* used, size_t* allocated,
  size_t* overhead)
#else
...
#ifdef USE_MEMTRACK
      *allocated += POOL_ALLOC_SIZE(chunk_t);
      *allocated += ponyint_pool_used_size(chunk->size);
      *overhead += sizeof(chunk_t);
#endif
      *used += chunk->size;
    } else {
  ...
  uint32_t sizeclass)
{
  ...
  if(chunk != NULL)
  {
    ...
    n->size = sizeclass;
#ifdef USE_MEMTRACK
    heap->overhead += sizeof(chunk_t);
    heap->allocated += POOL_ALLOC_SIZE(chunk_t);
    heap->allocated += POOL_ALLOC_SIZE(block_t);
    heap->overhead += POOL_ALLOC_SIZE(block_t);
#endif
...
#ifdef USE_MEMTRACK
  heap->overhead -= SIZECLASS_SIZE(sizeclass);
#endif
...
void* ponyint_heap_alloc_large(pony_actor_t* actor, heap_t* heap, size_t size)
{
  ...
  heap->overhead += sizeof(chunk_t);
  heap->allocated += POOL_ALLOC_SIZE(chunk_t);
  heap->allocated += ponyint_pool_used_size(size);
#endif
  chunk->slots = 0;
  chunk->shallow = 0;
...
#ifdef USE_MEMTRACK
    used += sweep_small(list1, avail, full, empty, size, &allocated, &overhead);
    used += sweep_small(list2, avail, full, empty, size, &allocated, &overhead);
#else
...
#ifdef USE_MEMTRACK
  heap->large = sweep_large(heap->large, &used, &allocated, &overhead);
#else
